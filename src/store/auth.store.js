...
export const useAuthStore = create((set) => ({
    ...
    initAuth: async () => {
        ...
        try {
            ...
            const profile = asRecord(unwrap(response));
            const user = {
    ...
    login: async (email, password) => {
        ...
        try {
            ...
            const accessToken = (inner.accessToken || payload.accessToken);
            const refreshToken = (inner.refreshToken || payload.refreshToken);
            if (!accessToken)
                throw new Error('No token in response');
            TokenStorage.setTokens(accessToken, refreshToken);
            const userFromResponse = (inner.user || payload.user);
