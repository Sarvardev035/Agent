...
const Accounts = () => {
    ...
    const ensureDestinationAccount = async (source) => {
        const preferred = accounts.find(a => String(a.id) !== String(source.id) && String(a.currency) === String(source.currency));
        if (preferred)
            return preferred;
        ...
        const createdAccount = created?.data;
        if (createdAccount?.id)
            return createdAccount;
        await refreshAccounts();
        const fromStore = useFinanceStore.getState().accounts.find(a => String(a.name) === fallbackName && String(a.currency) === String(source.currency));
        if (fromStore)
            return fromStore;
        throw new Error('Could not prepare destination account for balance transfer');
    };
    const moveBalanceBeforeDelete = async (source) => {
