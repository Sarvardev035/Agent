...
const Notes = () => {
  ...
  // Auto-save active note 500ms after typing stops
  useEffect(() => {
    if (!active) return
  ...
  const shareNote = async (note: Note) => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: note.title || 'My Finly Note', text })
      } catch {}
    } else {
  ...
  const togglePin = (id: string) => {
    ...
    setNotes(updated)
    saveNotes(updated)
    if (active?.id === id) setActive(prev => prev ? { ...prev, pinned: !prev.pinned } : prev)
  }
    ...
    <div className="page-content">
      <div style={{
        ...
        <div>
          <h1 style={{
            ...
            fontWeight: 800, color: 'var(--text-1)', margin: 0,
          }}>
            Notes 📝
          </h1>
      ...
      <div style={{
        ...
        gridTemplateColumns: active
          ? '260px 1fr' : 'repeat(auto-fill,minmax(220px,1fr))',
        gap: 16,
      }}>
          ...
          <div className="card" style={{ display: 'flex', flexDirection: 'column', minHeight: 400 }}>
            ...
            <div style={{
              ...
              <div style={{ fontSize: 11, color: 'var(--text-3)' }}>
                Auto-saving... <span style={{ marginLeft: 6 }}>💾</span>
              </div>
            ...
            <input
              value={active.title}
              onChange={e =>
                setActive(p => p ? { ...p, title: e.target.value } : p)}
            ...
            <textarea
              value={active.body}
              onChange={e =>
                setActive(p => p ? { ...p, body: e.target.value } : p)}
...
- Financial targets`}
            ...
            <div style={{
              ...
              {active.body.split(' ').filter(Boolean).length} words ·{' '}
              {active.body.length} characters · Saved locally on your device
            </div>
          </div>
        )}
      </div>
