...
const performTokenRefresh = async () => {
    ...
    if (!refreshPromise) {
        refreshPromise = axios
            .post(`${API_BASE_URL}/api/auth/token/refresh`, { refreshToken }, {
...
const bindBrowserNetworkEvents = () => {
    if (typeof window === 'undefined' || window.__FINLY_NETWORK_EVENTS_BOUND__)
        return;
...
api.interceptors.request.use(config => {
    ...
    config.__finlyRequestId = requestId;
    activeRequests += 1;
    if (typeof window !== 'undefined') {
...
api.interceptors.response.use(response => {
    ...
    const isAuthRequest = requestPath.includes('/api/auth/login')
